<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Travel Website</title>
    <link rel="stylesheet" href="assets/css/bootstrap.min.css">
    <link rel="stylesheet" href="assets/css/font-awesome.min.css">
    <link rel="stylesheet" href="assets/css/style.css">
</head>
<body>
    <header class="header-area">
        <div class="container">
            <div class="row">
                <div class="col-md-6">
                    <a href="" class="logo">
                        <img src="assets/img/logo.png" alt="" class="img-fluid">
                    </a>
                </div>
                <div class="col-md-6">
                    <a href="" class="top-img">
                        <img src="assets/img/tripadvisor-logo.png" alt="">
                    </a>
                </div>
            </div>
            <div class="row">
                <div class="col-md-12">
                    <nav class="navbar navbar-expand-lg">
                        <a class="navbar-brand" href="index.html">halim</a>
                        <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNavDropdown" aria-controls="navbarNavDropdown" aria-expanded="false" aria-label="Toggle navigation">
                          <span class="navbar-toggler-icon"></span>
                        </button>
                        <div class="collapse navbar-collapse" id="navbarNavDropdown">
                          <ul class="navbar-nav halim-menu">
                            <li class="nav-item">
                              <a class="nav-link" href="">Home</a>
                            </li>
                            <li class="nav-item">
                              <a class="nav-link" href="">About Us</a>
                            </li>
                            <li class="nav-item caret">
                              <a class="nav-link" href="">Bangladesh Information</a>
                              <ul>
                                  <li><a href="">Bangladesh Information</a></li>
                                  <li><a href="">Biography of Bangladesh</a></li>
                                  <li><a href="">Map of Bangladesh</a></li>
                                  <li><a href="">History of Bangladesh</a></li>
                                  <li><a href="">Weather of Bangladesh</a></li>
                                  <li><a href="">Festivals in Bangladesh</a></li>
                                  <li><a href="">Archaeological Sites in  Bangladesh</a></li>
                                  <li><a href="">Airlines of Bangladesh</a></li>
                                  <li><a href="">People &#038; Culture</a></li>
                                  <li><a href="">Geography and Climate</a></li>
                                  <li><a href="">Foreign Mission</a></li>
                                  <li><a href="">Visa Information</a></li>
                                  <li><a href="">Useful Information</a></li>
                              </ul>
                            </li>
                            <li class="nav-item">
                              <a class="nav-link" href="">Our Tours</a>
                            </li>
                            <li class="nav-item caret">
                              <a class="nav-link" href="">Services</a>
                                <ul>
                                    <li><a href="">Car Rental</a></li>
                                </ul>
                            </li>
                            <li class="nav-item">
                              <a class="nav-link" href="">Client Reviews</a>
                            </li>
                            <li class="nav-item">
                              <a class="nav-link" href="">Gallery</a>
                            </li>                              
                            <li class="nav-item">
                              <a class="nav-link" href="">Contact Us</a>
                            </li>
                          </ul>
                        </div>
                    </nav>
                </div>
            </div>
        </div>
    </header>

    <section class="services-area">
        <div class="container">
            <div class="row">
                <div class="col-md-12 text-center">
                    <div class="section-title">
                        <img src="assets/img/title-icon.png" alt="">
                        <h4>Bangladesh Information</h4>
                        <p>Bangladesh has an approximate population density of 1,084 per square kilometer (2,808 per square mile). </p>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="row">
                    <div class="col-lg-4 col-md-6">
                       <!-- Single Service -->
                       <div class="single-service">
                          <img src="assets/img/services/service-icon-1.png" alt="">
                          <h4>Biography</h4>
                          <p>Bangladesh, to the east of India on the Bay of Bengal, is a South Asian country marked by lush greenery.</p>
                          <a href="">read more</a>
                       </div>
                    </div>
                    <div class="col-lg-4 col-md-6">
                       <!-- Single Service -->
                       <div class="single-service">
                          <img src="assets/img/services/service-icon-2.png" alt="">
                          <h4>Map</h4>
                          <p>The People's Republic of Bangladesh, is a country in South Asia and converges with South-east. </p>
                          <a href="">read more</a>
                       </div>
                    </div>
                    <div class="col-lg-4 col-md-6">
                       <!-- Single Service -->
                       <div class="single-service">
                          <img src="assets/img/services/service-icon-3.png" alt="">
                          <h4>Visa Information</h4>
                          <p>Visa requirements for Bangladeshi citizens are administrative entry restrictions imposed on citizens of Bangladesh </p>
                          <a href="">read more</a>
                       </div>
                    </div>
                    <div class="col-lg-4 col-md-6">
                       <!-- Single Service -->
                       <div class="single-service">
                          <img src="assets/img/services/service-icon-4.png" alt="">
                          <h4>Foreign Mission</h4>
                          <p>This is a list of diplomatic missions in Bangladesh. At present, the capital city of Dhaka hosts 46 </p>
                          <a href="">read more</a>
                       </div>
                    </div>
                    <div class="col-lg-4 col-md-6">
                       <!-- Single Service -->
                       <div class="single-service">
                          <img src="assets/img/services/service-icon-5.png" alt="">
                          <h4>History</h4>
                          <p>Modern Bangladesh emerged as an independent nation in 1971 after breaking away and achieving independence from Pakistan</p>
                          <a href="">read more</a>
                       </div>
                    </div>
                    <div class="col-lg-4 col-md-6">
                       <!-- Single Service -->
                       <div class="single-service">
                          <img src="assets/img/services/service-icon-8.png" alt="">
                          <h4>Geography and Climate</h4>
                          <p>Bangladesh is a densely-populated, low-lying, mainly riverine country located in South Asia with a coastline of 580 km (360 mi). </p>
                          <a href="">read more</a>
                       </div>
                    </div>
                    <div class="col-lg-4 col-md-6">
                       <!-- Single Service -->
                       <div class="single-service">
                          <img src="assets/img/services/service-icon-9.png" alt="">
                          <h4>Weather</h4>
                          <p>Straddling the Tropic of Cancer, Bangladesh's climate is tropical with a mild winter from October to March, and a hot. </p>
                          <a href="">read more</a>
                       </div>
                    </div>
                    <div class="col-lg-4 col-md-6">
                       <!-- Single Service -->
                       <div class="single-service">
                          <img src="assets/img/services/service-icon-6.png" alt="">
                          <h4>People & Culture</h4>
                          <p>Bangladesh is ethnically homogeneous, with Bengalis comprising 98% of the population. Bangladesh is a Muslim-majority country.  </p>
                          <a href="">read more</a>
                       </div>
                    </div>
                    <div class="col-lg-4 col-md-6">
                       <!-- Single Service -->
                       <div class="single-service">
                          <img src="assets/img/services/service-icon-7.png" alt="">
                          <h4>Airlines</h4>
                          <p>Biman Bangladesh Airlines was established on 4 January 1972 as Bangladesh's national airline under the Bangladesh Biman Ordinance. </p>
                          <a href="">read more</a>
                       </div>
                    </div>
                </div>
            </div>
        </div>
    </section>

    <section class="packages-area">
        <div class="container">
            <div class="row">
                <div class="col-md-12 text-center">
                    <div class="section-title">
                        <img src="assets/img/title-icon.png" alt="">
                        <h4>Bangladesh Information</h4>
                        <p>Bangladesh has an approximate population density of 1,084 per square kilometer (2,808 per square mile). </p>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-md-4">
                    <div class="single-package">
                        <img src="assets/img/package1.png" alt="">
                        <h4>National Capitol of Cuba </h4>
                        <div class="package-meta">
                            <span><i class="bx bx-map"></i> USA</span>
                            <span><i class="bx bxs-watch"></i> 3 Days / 4 Nights</span>
                            <span><i class="bx bx-money"></i>$59</span>    
                        </div>
                        <a href="">explore now <i class="bx bxs-right-arrow-circle"></i></a>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="single-package">
                        <img src="assets/img/package1.png" alt="">
                        <h4>National Capitol of Cuba </h4>
                        <div class="package-meta">
                            <span><i class="bx bx-map"></i> USA</span>
                            <span><i class="bx bxs-watch"></i> 3 Days / 4 Nights</span>
                            <span><i class="bx bx-money"></i>$59</span>    
                        </div>
                        <a href="">explore now <i class="bx bxs-right-arrow-circle"></i></a>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="single-package">
                        <img src="assets/img/package1.png" alt="">
                        <h4>National Capitol of Cuba </h4>
                        <div class="package-meta">
                            <span><i class="bx bx-map"></i> USA</span>
                            <span><i class="bx bxs-watch"></i> 3 Days / 4 Nights</span>
                            <span><i class="bx bx-money"></i>$59</span>    
                        </div>
                        <a href="">explore now <i class="bx bxs-right-arrow-circle"></i></a>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="single-package">
                        <img src="assets/img/package1.png" alt="">
                        <h4>National Capitol of Cuba </h4>
                        <div class="package-meta">
                            <span><i class="bx bx-map"></i> USA</span>
                            <span><i class="bx bxs-watch"></i> 3 Days / 4 Nights</span>
                            <span><i class="bx bx-money"></i>$59</span>    
                        </div>
                        <a href="">explore now <i class="bx bxs-right-arrow-circle"></i></a>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="single-package">
                        <img src="assets/img/package1.png" alt="">
                        <h4>National Capitol of Cuba </h4>
                        <div class="package-meta">
                            <span><i class="bx bx-map"></i> USA</span>
                            <span><i class="bx bxs-watch"></i> 3 Days / 4 Nights</span>
                            <span><i class="bx bx-money"></i>$59</span>    
                        </div>
                        <a href="">explore now <i class="bx bxs-right-arrow-circle"></i></a>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="single-package">
                        <img src="assets/img/package1.png" alt="">
                        <h4>National Capitol of Cuba </h4>
                        <div class="package-meta">
                            <span><i class="bx bx-map"></i> USA</span>
                            <span><i class="bx bxs-watch"></i> 3 Days / 4 Nights</span>
                            <span><i class="bx bx-money"></i>$59</span>    
                        </div>
                        <a href="">explore now <i class="bx bxs-right-arrow-circle"></i></a>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <script src="assets/js/jquery.min.js"></script>
    <script src="assets/js/main.js"></script>
</body>
</html>